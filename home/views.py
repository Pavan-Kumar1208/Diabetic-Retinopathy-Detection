...
model = tf.keras.models.load_model(path,custom_objects={'KerasLayer':hub.KerasLayer, 'K':K})
...
def output(request):
    if request.method=='POST':
        ...
        gray_image = cv2.cvtColor(input_image_resize, cv2.COLOR_BGR2GRAY)
        input_image_scaled = gray_image/255
        image_reshaped = np.reshape(input_image_scaled, [1,128,128,1])
